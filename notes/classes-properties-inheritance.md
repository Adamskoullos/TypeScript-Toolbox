...
Using `protected` instead of `private` on the properties of a base class allows extended classes to mutate the property, whilst also preventing public access.
...
Static properties and methods can be used when we do need a new instance:

```ts
class Coupon {
  static allowed  ['cheese', 'ham'];
  static create(percent: number){
    return `pizza_discount_${percent}`;
  }
}
```
